...
const sendConfirmationEmail = async (appointmentId) => {
  try {
    const appointment = await Appointment.findById(appointmentId)
      ...
      .populate("patientProfile", "fullName email");
    if (!appointment) {
    ...
    if (!appointment.doctor?.fullName) {
      logger.warn(`Không có thông tin doctor cho appointment ${appointmentId}`);
      return;
    }
...
export const processRefund = async (paymentId, refundAmount, reason) => {
  ...
  try {
    ...
    if (refundResult.responseCode === "00") {
      // ✅ FIX: Cập nhật refund status + TRỪ tiền khỏi payment.amount
      payment.refundStatus = "completed";
      payment.refundAmount = refundAmount;
      payment.refundedAt = new Date();
      payment.refundTransactionId = refundResult.refundTransactionId;

      // 🔥 THÊM: Trừ tiền đã hoàn từ số tiền gốc
      ...
      payment.refundAmount = 0; // ✅ FIX: Reset về 0 nếu hoàn tiền thất bại
      await payment.save();
      throw new ApiError(
    ...
    // ✅ FIX: Đảm bảo status reset về "failed" nếu có exception
