...
import { Avatar, Badge, Button, Input, Tag, Tooltip } from "antd";
...
const MedicalChatbot = () => {
  ...
  // 1. Logic khởi tạo sessionId thông minh
  const [sessionId, setSessionId] = useState(() => {
    const { isAuthenticated } = useAuthStore.getState(); // Lấy trực tiếp từ store để khởi tạo

    // NẾU đã đăng nhập: Ưu tiên lấy ID cũ từ localStorage để duy trì phiên khi reload
    if (isAuthenticated) {
      const stored = localStorage.getItem("authenticatedChatSessionId");
      if (stored) return stored;
    }

    // NẾU là khách (hoặc chưa có ID lưu trữ): Sinh ID mới (F5 sẽ tự mất vì không lưu localStorage)
    return crypto.randomUUID
      ? crypto.randomUUID()
      : Math.random().toString(36).substring(2);
  });

  // 2. Theo dõi thay đổi trạng thái đăng nhập
  useEffect(() => {
    const handleSessionSwitch = () => {
      // Luôn sinh ID mới khi có sự thay đổi trạng thái (Login/Logout)
      const newId = crypto.randomUUID
        ? crypto.randomUUID()
        : Math.random().toString(36).substring(2);

      if (isAuthenticated) {
        // Khi Đăng nhập: Lưu vào localStorage để "chốt" 1 phiên duy nhất cho User
        localStorage.setItem("authenticatedChatSessionId", newId);
      } else {
        // Khi Đăng xuất: Xóa sạch dấu vết session của User
        localStorage.removeItem("authenticatedChatSessionId");
      }

      setSessionId(newId);

      // Reset tin nhắn hiển thị về trạng thái ban đầu
      setMessages([
        {
          id: Date.now(),
